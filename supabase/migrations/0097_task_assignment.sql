-- ============================================================
-- 0097_task_assignment.sql
-- TASK-ASSIGN-01: checklist task ownership per project.
-- assigned_to rides existing can_access_project SELECT /
-- can_edit_project write policy — no new RLS on tasks.
-- Clone paths explicitly null assigned_to (never copy it).
--
-- Next-free after 0096_automation_templates.sql.
-- ============================================================
...
create or replace function list_project_assignees(p_project_id uuid)
returns table (user_id uuid, email text, role_label text)
language plpgsql
security definer
...
begin
  ...
  return query
    select
      ...
      u.email::text,
      case
        when v_account_kind = 'business' then 'planner'
        else 'couple'
      end
...
grant execute on function list_project_assignees(uuid) to authenticated;

-- TMPL-01: never copy assigned_to from the source project.
...
-- DEMO clone: body from 0074 with assigned_to explicitly nulled on task copy.
...
begin
  ...
  -- assigned_to is explicitly null — never copied from the template.
  insert into tasks (
    id, project_id, title, status, phase, due_date, vendor_id, position, notes, created_at, assigned_to
  )
  select
    m.new_id,
    pm.new_id,
